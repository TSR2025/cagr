...
export function InputsPanel({ inputs, onChange }: InputsPanelProps) {
  ...
  const hasInitialBoosts = (inputs.boosts || []).some((boost) => boost.amount || boost.year || boost.label);
  const [boostMode, setBoostMode] = useState<"none" | "add">(hasInitialBoosts ? "add" : "none");
  const [savedBoosts, setSavedBoosts] = useState<OneTimeBoost[]>(inputs.boosts || []);
  const [showBoosts, setShowBoosts] = useState(hasInitialBoosts);
    ...
    <Card className="sticky top-6 h-fit w-full max-w-[420px] border-slate-200 bg-white/90 shadow-subtle">
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 shadow-subtle">
          ...
          <div className="space-y-2 rounded-2xl border border-slate-200/90 bg-white/80">
            <button
              type="button"
              onClick={() => setShowBoosts((prev) => !prev)}
              className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-semibold text-slate-800 transition hover:text-slate-900"
            ...
            <div
              ...
              style={{ maxHeight: showBoosts ? 1200 : 0, transition: `max-height ${showBoosts ? 300 : 240}ms ${EASING}` }}
            >
              <div
                className={clsx(
